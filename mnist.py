...
tf.flags.DEFINE_string("data_dir", "/users/zihangm/github/mesh/examples/mnist_data",
                       "Path to directory containing the MNIST dataset")
tf.flags.DEFINE_string("model_dir", "/users/zihangm/github/mesh/examples/mnist_model", "Estimator model_dir")
...
def mnist_model(image, labels, mesh):
 ...
 # row_blocks_dim_layer1 = mtf.Dimension("row_blocks_layer1", 4)
  col_blocks_dim = mtf.Dimension("col_blocks", 4)
 # col_blocks_dim_layer1 = mtf.Dimension("col_blocks_layer1", 4)
  ...
  row_blocks_dim_new = mtf.Dimension("row_blocks_new", 4)
  col_blocks_dim_new = mtf.Dimension("col_blocks_new", 4)
  rows_dim_new = mtf.Dimension("rows_size_new", 7)
  cols_dim_new = mtf.Dimension("cols_size_new", 7)

  new_shape = mtf.Shape([batch_dim, row_blocks_dim_new, col_blocks_dim_new,
           rows_dim_new, cols_dim_new])
  ...
  hidden_dim1_new = mtf.Dimension("hidden1_new", FLAGS.hidden_size)
  new_shape_h1 = mtf.Shape([batch_dim, hidden_dim1_new])
  h1 = mtf.reshape(h1, new_shape_h1)
...
def model_fn(features, labels, mode, params):
  ...
  #print('!!!!!!mesh_size', mesh_size, mesh_shape)
  #a=1;b=1;c=1
  devices = [""] * a*b*c*d*e*f*g*h*i
  mesh_shape = [("b1", a), ("b2", b), ("b3", c), ("b4",d), ("b5",e), ("b6",f), ("b7",g), ("b8",h),("b9",i)]
  layout_rules = [("filters1", "b1"), ("filters2", "b2"), ("hidden1", "b3"), ("hidden2", "b4"), ("row_blocks_new", "b5"), ("hidden1_new", "b6"), ("batch","b7"), ("row_blocks", "b8"),("col_blocks","b9")]
...
def run_mnist():
  ...
  for _ in range(FLAGS.train_epochs // FLAGS.epochs_between_evals):
    ...
    with open("time.txt", "a") as fp: 
        fp.write("data("+str(a)+";"+str(b)+";"+str(c)+";"+str(d)+";"+str(e)+";"+str(f)+";"+str(g)+";"+str(h)+";"+str(i)+"):"+str(time_end - time_start) + "\n")
    
    print ('\n\n\n!~!~!~!~!~!~!~!~time for one epoch:!~!~!~!~!~!~!~!~', time_end - time_start)
    sys.exit(0)
